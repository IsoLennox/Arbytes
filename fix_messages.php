...
if (isset($_POST['submit'])) {
  ...
  if (empty($errors) ) {
      ...
      //get thread id that you are involved in
        $thread_query  = "SELECT * FROM thread WHERE (user1={$send_to} OR user2={$send_to}) AND (user1={$_SESSION['user_id']} OR user2={$_SESSION['user_id']})"; 
      ...
      if (!empty($thread_retrieved_array)) {
        //YOU HAVE THREADS WITH THIS PERSON
    ...
    foreach($thread_retrieved as $check_interaction){
          ...
          $_SESSION["message"] = "you have a thread with this person";
...
//insert message went here before check_interaction was created
          ...
          $_SESSION["message"] = "You do not any have threads with this person!";
      
//      YOU DO NOT HAVE A THREAD 
//          
//          CREATE THREAD
...
//    $email = $member['email'];
//    $message = "Hi, ".$name."! You have a new message from ".$_SESSION['first_name']." ".$_SESSION['last_name']."!";
//    $formcontent=$message." View your messages";
//    $recipient = "contact@isobellennox.com";
//    $subject = "New Message on Arbytes!";
//    $mailheader = $message;
//    mail($recipient, $subject, $formcontent, $mailheader) or die("Error!");
            
            
$send_to_name = "Isobel";
$email_address = "isolennox@gmail.com";
$email_message = "SUCCESS!";
$formcontent="From: $send_to_name \n Message: $email_message";
$recipient = "isolennox@gmail.com";
...
?> 
  ...
  <div id="page">
    ...
    <h2>Send Message</h2>
    <form action="fix_messages.php?user_id=<?php echo $send_to; ?>&name=<?php echo $name; ?>" method="post">
 
        <p>Send to <?php echo $name ?>:
